...
export default function ProcessedProductPage() {
  ...
  const handleSubmitInquiry = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.email || !formData.message) {
      return alert('请填写所有必填项。'); // 모든 항목을 입력해주세요.
    }
    ...
    try {
      ...
      alert('您的咨询已成功受理。相关负责人确认后，会尽快与您联系！'); // 문의 접수 성공
      setIsModalOpen(false);
    } catch (error) {
      alert('咨询受理过程中出现错误，请稍后再试。'); // 문의 접수 오류
    } finally {
    ...
    <div className="w-full min-h-screen bg-gray-50 pb-24 pt-20">
      ...
      <section className="relative w-full h-[30vh] bg-slate-800 flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-20 text-center px-4">
          ...
          <p className="text-blue-100 text-lg font-light">征服全球味蕾，品鉴多元化韩式美食</p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-12">
          {['All', 'Ambient', 'Frozen'].map((tab) => (
            ...
            >
              {tab === 'All' ? '全部查看' : tab === 'Ambient' ? '常温 (Ambient)' : '冷冻 (Frozen)'}
            </button>
          ))}
        </div>
        ...
        {isLoading ? (
          <div className="w-full py-32 flex flex-col items-center justify-center text-blue-600">
             ...
             <p className="text-gray-500 font-medium">正在实时读取加工产品数据...</p>
          </div>
        ) : (
          /* 🥫 제품 그리드 진열대 */
        ...
        {!isLoading && filteredProducts.length === 0 && (
          <div className="w-full py-32 text-center flex flex-col items-center">
            ...
            <p className="text-gray-500 text-lg">暂无已登记的加工产品。<br/>请在管理后台注册产品！</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all">
            ...
            <form onSubmit={handleSubmitInquiry} className="p-6">
              <div className="mb-6 bg-blue-50 p-4 rounded-lg border border-blue-100 flex items-center space-x-4">
                ...
                <div>
                  <p className="text-xs font-bold text-blue-600 mb-1">已选产品</p>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">公司名称 / 联系人姓名 <span className="text-blue-600">*</span></label>
                  <input type="text" name="name" value={formData.name} onChange={handleInputChange} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-shadow" placeholder="例如：Evergood / 张三" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">回复邮箱 <span className="text-blue-600">*</span></label>
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">咨询内容 (数量、交期等) <span className="text-blue-600">*</span></label>
                  <textarea name="message" value={formData.message} onChange={handleInputChange} required rows={4} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none resize-none transition-shadow" placeholder="请填写所需数量、希望交货期及其他具体要求。" />
                </div>
              </div>
