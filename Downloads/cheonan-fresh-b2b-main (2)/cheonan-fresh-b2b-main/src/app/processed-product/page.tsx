    ...
    <div className="w-full min-h-screen bg-gray-50 pb-24 pt-20">
      ...
      <section className="relative w-full h-[30vh] bg-slate-800 flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-20 text-center px-4">
          ...
          <p className="text-blue-100 text-lg font-light">전 세계인의 입맛을 사로잡은 다채로운 K-푸드의 맛</p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-12">
          {['All', 'Ambient', 'Frozen'].map((tab) => (
            ...
            >
              {tab === 'All' ? '전체 보기' : tab === 'Ambient' ? '상온 (Ambient)' : '냉동 (Frozen)'}
            </button>
          ))}
        </div>
        ...
        {isLoading ? (
          <div className="w-full py-32 flex flex-col items-center justify-center text-blue-600">
             ...
             <p className="text-gray-500 font-medium">가공제품 데이터를 안전하게 불러오는 중입니다...</p>
          </div>
        ) : (
          /* 🥫 제품 그리드 진열대 */
        ...
        {!isLoading && filteredProducts.length === 0 && (
          <div className="w-full py-32 text-center flex flex-col items-center">
            ...
            <p className="text-gray-500 text-lg">아직 등록된 가공제품이 없습니다.<br/>관리자 페이지에서 제품을 등록해 주세요!</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all">
            ...
            <form onSubmit={handleSubmitInquiry} className="p-6">
              <div className="mb-6 bg-blue-50 p-4 rounded-lg border border-blue-100 flex items-center space-x-4">
                ...
                <div>
                  <p className="text-xs font-bold text-blue-600 mb-1">선택된 제품</p>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">회사명 / 담당자 성함 <span className="text-blue-600">*</span></label>
                  <input type="text" name="name" value={formData.name} onChange={handleInputChange} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-shadow" placeholder="예: 에버굿 / 홍길동" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">회신받을 이메일 <span className="text-blue-600">*</span></label>
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">문의 내용 (수량, 납기 등) <span className="text-blue-600">*</span></label>
                  <textarea name="message" value={formData.message} onChange={handleInputChange} required rows={4} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none resize-none transition-shadow" placeholder="필요하신 수량, 희망 납기일, 기타 요청사항을 적어주세요." />
                </div>
              </div>
