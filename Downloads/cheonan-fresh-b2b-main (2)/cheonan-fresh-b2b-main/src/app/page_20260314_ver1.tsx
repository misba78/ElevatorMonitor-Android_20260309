...
export default function HomePage() {
  ...
  const [partners, setPartners] = useState<any[]>([]); // 🌟 파트너(협력사) 상태 추가
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setPartners(partnerData || []); // 🌟 상태에 파트너 데이터 저장
      setIsLoading(false);
    };
    fetchData();
  }, []);
  ...
  return (
    ...
    {/* 1. Hero Section (모바일 최적화 완벽 적용) */}
      <section className="relative w-full min-h-[85vh] py-32 md:py-0 bg-green-900 flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 max-w-5xl mx-auto flex flex-col items-center mt-12 md:mt-16 w-full">
          <span className="px-3 py-1.5 rounded-full bg-green-800 border border-green-600 text-green-300 text-[11px] md:text-sm font-bold tracking-widest mb-6 uppercase">PREMIUM KOREAN FOOD EXPORTER</span>
          
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-black text-white mb-6 leading-tight tracking-tight px-2">
          ...
          <p className="text-base md:text-xl text-green-100 mb-10 max-w-2xl font-light leading-relaxed px-4">
      ...
      <section className="py-24 bg-gray-50 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-end mb-12">
            ...
            <div className="hidden md:block text-green-600 font-bold hover:underline cursor-pointer tracking-tighter uppercase text-sm">View All News</div>
          </div>
      ...
      {/* 🌟 4. Global Partners Section (새로 추가됨!) */}
