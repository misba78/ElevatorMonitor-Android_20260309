...
// 🌟 애니메이션 라이브러리 (AnimatePresence 추가)
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    fetchData();
    return () => clearInterval(langTimer); // 메모리 누수 방지
  }, []);
        ...
        >
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {lang === 'ko' 
                ...
                : "Cheonan Fresh delivers Korea's premium produce worldwide through rigorous quality control and cold-chain systems."}
            </motion.p>
          </AnimatePresence>
          ...
          <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto px-6 sm:px-0">
            <Link href="/fresh-product" className="w-full sm:w-auto px-8 py-4 sm:py-5 bg-green-500 hover:bg-green-400 text-white rounded-2xl sm:rounded-full font-black text-lg shadow-xl shadow-green-900/50 flex items-center justify-center transition-all transform hover:-translate-y-1">
              {lang === 'ko' ? '제품 둘러보기' : 'View Products'} <ArrowRight className="ml-2" size={20} />
            </Link>
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      {!isLoading && news.length > 0 ? news.map((item) => (
        /* 🌟 변경 포인트: 전체 카드를 Link로 감싸 클릭 시 상세 페이지(/news/[id])로 이동하게 함 */
        <Link 
          href={`/news/${item.id}`} 
          key={item.id} 
        ...
        >
          ...
          <div className="p-8 flex flex-col flex-grow">
            ...
            {/* 🌟 변경 포인트: Read More 부분에 시각적 효과 유지 및 부모 Link 작동 */}
...
<section className="py-24 px-4 max-w-7xl mx-auto">
  <div className="text-center mb-16">
    ...
    {/* 뉴스 링크와 혼동되지 않도록 제품 전체보기 링크로 수정 (필요 시 경로 확인) */}
  ...
  <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
    {!isLoading && latestProducts.map((product, index) => (
      /* 🌟 변경 포인트: 제품 카드 전체를 Link로 감싸 상세 페이지(/product/[id])로 이동 */
      <Link 
      ...
      >
        ...
        <div className="p-10">
          ...
          {/* 🌟 변경 포인트: '자세히 보기' 부분 시각적 효과 유지 (부모 Link가 이동 담당) */}
